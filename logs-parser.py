...
def decode_body(encoding: int, body: str) -> str:
...
def process_logs(input_lines: list[str]) -> None:
    ...
    for pipeline_id, pipeline_msgs in messages_by_pipeline.items():
        ...
        if len(last_msgs) == 0:
            error_msg = f"Pipeline {pipeline_id} has no last message (next_id == \"-1\")."
            logging.error(error_msg)
            sys.stderr.write(f"Error: {error_msg}\n")  # Optional
            continue
        ...
        if sequence:
            ...
            if orphan_ids:
                ...
                logging.warning(warning_msg)  # Warnings also logged
...
def main():
    ...
    # Set up logging with the specified or default log file
